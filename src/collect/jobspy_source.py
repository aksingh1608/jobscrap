...
def collect_jobspy(cfg: dict[str, Any], site_names: list[str]) -> list[JobRecord]:
    ...
    freshness_days = int(cfg.get("freshness_days") or 7)
    hours_old = freshness_days * 24
    queries = cfg.get("search_queries") or ["AI internship"]
    results_per_query = max(20, 300 // max(len(queries), 1))
    ...
    for q in queries:
        try:
            log.info("JobSpy: query=%r sites=%s location=%s", q, sites, location)
            df = scrape_jobs(
                site_name=sites,
                search_term=q,
                location=location,
                ...
                verbose=0,
                # Prefer internship when the board supports job_type
                job_type="internship",
            )
        except Exception:
            # Some boards reject job_type + hours_old together — retry without job_type
            log.warning("JobSpy retry without job_type for query=%r", q, exc_info=True)
            try:
                df = scrape_jobs(
                    site_name=sites,
                    search_term=q,
                ...
                log.exception("JobSpy failed for query=%r", q)
                continue
